...
GameController::GameController(QWidget *parent) : QMainWindow(parent) {
  resize(1100, 750);
  applyDarkTheme();
...
void GameController::applyDarkTheme() {
...
void GameController::initSongSelect() {
  QWidget *w = new QWidget();
...
void GameController::filterSongs(const QString &query) {
...
void GameController::refreshSongList() {
  listSongs->clear();
...
void GameController::initSettings() {
  QWidget *w = new QWidget();
  QVBoxLayout *l = new QVBoxLayout(w);
  l->setContentsMargins(40, 40, 40, 40);
  ...
  QCheckBox *chkNoLongNotes = new QCheckBox("NO LONG NOTES MODE");
  chkNoLongNotes->setChecked(SettingsManager::instance().isNoLongNotes());
  chkNoLongNotes->setStyleSheet("color: #CCFF00; font-weight: bold; margin-bottom: 5px;");
  connect(chkNoLongNotes, &QCheckBox::toggled,
          [](bool v) { SettingsManager::instance().setNoLongNotes(v); });
  l->addWidget(chkNoLongNotes);

  QCheckBox *chkExperimental = new QCheckBox("EXPERIMENTAL AUTO-CHARTING");
  chkExperimental->setChecked(SettingsManager::instance().isExperimentalMode());
  chkExperimental->setStyleSheet("color: #CCFF00; font-weight: bold; margin-bottom: 20px;");
  connect(chkExperimental, &QCheckBox::toggled,
          [](bool v) { SettingsManager::instance().setExperimentalMode(v); });
  l->addWidget(chkExperimental);
...
void GameController::refreshLeaderboard() {
...
void GameController::initGameOver() {
  QWidget *w = new QWidget();
  QVBoxLayout *l = new QVBoxLayout(w);
